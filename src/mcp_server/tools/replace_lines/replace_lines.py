...
@registry.register(
    ...
    input_schema={
        ...
        "properties": {
            "file_path": {
                "type": "string",
                "description": "要修改的檔案路徑。只支援絕對路徑，例如 '/home/user/project/src/main.py'"
            },
            "start_line": {
                "type": "integer",
                "minimum": 1,
                "description": "開始行號（1-based，包含）"
            },
            "end_line": {
                "type": "integer",
                "minimum": 1,
                "description": "結束行號（1-based，包含），必須大於等於 start_line"
            },
            "new_content": {
                "type": "string",
                "description": "用於替換的新內容（可以是多行文字）"
            },
            "dry_run": {
                "type": "boolean",
                "default": False,
                "description": "預覽模式：顯示修改前後的差異，但不實際寫入檔案"
            },
            "validate_syntax": {
                "type": "boolean",
                "default": False,
...
async def handle_replace_lines(args: dict[str, Any]) -> ExecutionResult:
    ...
    return await replace_file_lines(
        file_path, start_line, end_line, new_content, dry_run, validate_syntax
    )
...
async def replace_file_lines(
    ...
    try:
        ...
        if start_line > total_lines:
            raise ValueError(
                f"start_line ({start_line}) 超過檔案總行數 ({total_lines})"
            )
        ...
        # 生成 diff 預覽
        diff_output = _generate_diff(
            target_path, original_content, new_content,
        ...
        logger.info(
            f"成功替換檔案 {target_path} 行 {start_line}-{actual_end_line}，"
            f"原始 {original_line_count} 行 -> 新 {new_line_count} 行"
        )
